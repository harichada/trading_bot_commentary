"""Shadow LONG trade resolver — replays hypothetical entries against bars.

v-shadow-long-resolver-2026-09-17. Mirrors the SHORT resolver
(shadow_short_resolver.py) for LONG mean-reversion entries. Loads
shadow_long_log.ndjson ledger or backtest trades, fetches subsequent
minute bars, and resolves each entry to stop/target/timeout/flatten.
...
logger = logging.getLogger("shadow_long_resolver")
...
@dataclass
class ShadowEntry:
    """Parsed row from shadow_long_log.ndjson."""
    timestamp: datetime
    symbol: str
    signal_type: str
    reason: str
    ...
    def from_dict(cls, d: dict) -> "ShadowEntry":
        ...
        return cls(
            ...
            signal_type=str(d.get("signal_type", "LONG")).upper(),
            reason=str(d.get("reason", "")),
            signal_close=float(d.get("signal_close", d.get("entry_price", 0))),
            rsi=float(d.get("rsi", d.get("rsi_14", 0))),
            ...
            hypothetical_stop=float(d.get("hypothetical_stop", d.get("stop_loss", 0))),
            hypothetical_target=float(d.get("hypothetical_target", d.get("take_profit", 0))),
...
def _default_log_paths() -> list[Path]:
    ...
    return [
        REPO_ROOT / "shadow_long_log.ndjson",
        REPO_ROOT / "data" / "shadow_long_log.ndjson",
    ]


def load_shadow_log(path: Optional[Path] = None) -> list[ShadowEntry]:
    """Load and parse the shadow_long_log.ndjson file."""
    if path is not None:
    ...
    if log_path is None:
        logger.warning("No shadow_long_log.ndjson found at: %s", candidates)
        return []
...
def resolve_entry(
    ...
    Entry price = signal_close - conservative slip (0.1% adverse for LONG).
    Barriers: hypothetical_stop (below), hypothetical_target (above).
    Time stop: 60 bars OR flatten at 15:55 ET — whichever comes first.
    R = (exit - entry) / stop_dist for LONG (positive R = profit).
    """
    stop = entry.hypothetical_stop
    target = entry.hypothetical_target
    stop_dist = entry.stop_dist or abs(entry.signal_close - stop)
    ...
    falling_knife_pass = entry.raw.get("falling_knife_pass", True)
    ...
    if df.empty or len(df) == 0:
        return ResolvedTrade(
            ...
            return_pct=0.0,
            regime=regime,
            falling_knife_pass=falling_knife_pass,
            raw_entry=entry.raw,
        )
    ...
    if stop_dist > 0 and exit_price is not None:
        r_multiple = (exit_price - entry_price) / stop_dist
    else:
        r_multiple = 0.0
    ...
    with open(path, "w") as f:
        for t in trades:
            row = {
                ...
                "regime": t.regime,
                "falling_knife_pass": t.falling_knife_pass,
            }
...
def print_summary(summary: StageASummary, label: str = "Promotion Book") -> None:
    ...
    if summary.all_gates_pass:
        print(f"\n  → ALL GATES PASS. Shadow data supports enabling MEAN_REV_LONG.")
    else:
        print(f"\n  → GATE(S) FAILED. Do NOT enable live LONG yet.")
    print(bar)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to shadow_long_log.ndjson. Default: repo-root then data/",
    )
    parser.add_argument(
        "--dsn",
        default=None,
