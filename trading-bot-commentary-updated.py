...
class BacktestEngine:
    ...
    def run_backtest(self, data: Dict[str, pd.DataFrame], strategy, 
        ...
        # Filter data by date range and get all unique dates
        filtered_data = {}
        all_dates = set()
        ...
        for symbol, df in data.items():
            ...
            all_dates.update(filtered_df.index.date)
        ...
        for date in all_dates:
            ...
            self._process_date(date, filtered_data, strategy)
    ...
    def _process_date(self, date, data: Dict[str, pd.DataFrame], strategy):
        ...
        for symbol, df in data.items():
            ...
            date_data = df[df.index.date == date]
            if not date_data.empty:
                # Create market data structure for strategy
                market_data = {symbol: df[df.index.date <= date]}
    ...
    def _execute_signal(self, signal, current_data):
        ...
        price = current_data['Close']  # Use 'Close' instead of 'close'
    ...
    def _calculate_sharpe_ratio(self, returns: pd.Series) -> float:
        ...
        if len(returns) == 0:
            return 0
        # Use default risk-free rate since config doesn't have performance_metrics
...
class TradingEngineWithCommentary:
    ...
    async def _close_real_position(self, position, exit_portion: float = 1.0):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                # Wait for order fill and get actual execution price
                actual_fill_price = await self._wait_for_order_fill_and_get_price(order_id, position.symbol)
                
                if actual_fill_price:
                    # Update position with actual fill price
                    position.current_price = actual_fill_price
                    ...
                    self.commentary.add_commentary(TradingCommentary(
                        ...
                        symbol=position.symbol,
                        title=f"✅ Order Filled",
                        message=f"Position closed at ${actual_fill_price:.2f} (actual execution price)",
                        data={'execution_price': actual_fill_price},
                        importance=8
                    ))
    ...
    async def _wait_for_order_fill_and_get_price(self, order_id: str, symbol: str, max_wait_seconds: int = 30) -> Optional[float]:
        """Wait for order to fill and return the actual execution price"""
        import asyncio
        import time
        ...
        while time.time() - start_time < max_wait_seconds:
            try:
                # Check order status
                order_status = await self._check_specific_order_status(order_id)
                
                if order_status == 'FILLED':
                    # Get order details to find fill price
                    response = self.schwab_client.get_order(order_id)
                    if response.status_code == 200:
                        order_data = response.json()
                        
                        # Extract fill price from order details
                        # This depends on Schwab API structure - adjust as needed
                        if 'orderActivityCollection' in order_data:
                            activities = order_data['orderActivityCollection']
                            if activities and len(activities) > 0:
                                # Get the execution price from the first activity
                                execution_price = activities[0].get('executionLegs', [{}])[0].get('price', 0)
        ...
        # If we couldn't get the actual fill price, log a warning
        logger.warning(f"Could not determine actual fill price for order {order_id}")
        return None
            ...
            if response.status_code == 200:
                ...
                for pos_data in positions_data:
                    ...
                    if symbol and symbol in self.positions:
                        ...
                        real_market_value = pos_data.get('marketValue', 0)
                        real_quantity = pos_data.get('longQuantity', 1)
                        real_unrealized_pnl = pos_data.get('unrealizedProfitLoss', 0)
                        
                        # Calculate real current price
                        if real_quantity > 0:
                            real_current_price = real_market_value / real_quantity
                            
                            # Validate against our tracked price
                            price_diff_pct = abs((real_current_price - position.current_price) / position.current_price) * 100
                            if price_diff_pct > 10:  # More than 10% difference
                                logger.warning(f"Price discrepancy for {symbol}: tracked=${position.current_price:.2f}, real=${real_current_price:.2f} ({price_diff_pct:.1f}%)")
                                
                                # Update with real price
                                position.current_price = real_current_price
                                position.unrealized_pnl = real_unrealized_pnl
                                
                                self.commentary.add_commentary(TradingCommentary(
                                    timestamp=datetime.now(),
                                    type=CommentaryType.WARNING,
                                    symbol=symbol,
                                    title=f"⚠️ Price Correction",
                                    message=f"Updated {symbol} price from ${position.current_price:.2f} to ${real_current_price:.2f}",
                                    data={'old_price': position.current_price, 'new_price': real_current_price},
                                    importance=7
                                ))
            ...
            try:
                ...
                if self.data_provider:
                    ...
                    if quote and quote.get('last'):
                        ...
                        # Validate price change is reasonable (not more than 50% in one update)
                        price_change_pct = abs((current_price - position.current_price) / position.current_price) * 100
                        if price_change_pct > 50:
                            logger.warning(f"Suspicious price change for {symbol}: {position.current_price} -> {current_price} ({price_change_pct:.1f}%)")
                            # Use a more conservative price update
                            current_price = position.current_price * (1 + np.sign(current_price - position.current_price) * 0.05)
                    else:
...
# Replace the existing run_backtest endpoint with this:
